...
/**
 * The ItemContainer class creates a list of items that can hold different types of items. The
 * container can be used to hold all items found in a room and all items in a player's inventory.
 * 
...
public class ItemContainer<I extends Item> {
  ...
  public ArrayList<I> getContainer() {
    return container;
  } // Getter

  public void setItem(ArrayList<I> container) {
  ...
  /**
   * This method is used when player finds an item. The item is added to the player's inventory if
  ...
  /**
   * This method is used when the player uses an item. The item is removed from the player's
   * inventory.
   * 
  ...
  /**
   * This method is used when the player inspects an item and prints the item description to the
   * console.
   */
  ...
  public I getItem(String name) {
    // Returns the item at a specific index
    return container.stream().filter(i -> (name.equals(i.getName()))).findAny().orElse(null);
  }
